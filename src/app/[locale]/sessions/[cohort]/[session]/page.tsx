...
type Props = { params: Promise<{ locale: Locale; cohort: string; session: string }> };
  ...
  return locales.flatMap((locale) =>
    getCohorts().flatMap((c) =>
      getSessions(c.slug)
        ...
        .map((s) => ({ locale, cohort: c.slug, session: s.slug })),
    ),
  );
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { locale, cohort, session } = await params;
  const s = getSession(cohort, session);
  if (!s) return {};
  return {
...
export default async function SessionPage({ params }: Props) {
  const { locale, cohort, session: slug } = await params;
